...
export const Login = (data, navigate) => {
  ...
  return async (dispatch) => {
    try {
      ...
      response.data.user1.role===Roles.Admin?navigate("/admin/dashboard"):response.data.user1===Roles.Student?navigate("/student/dashboard"):navigate("/dashboard");
    } catch (err) {
...
export const registerVendor=(data,navigate)=>{
  ...
  return async(dispatch)=>{
    try{
      const response=await apiConnector(AuthApi.RegisterVendorapi,"POST",data);
      if(!response.data.success){
        throw new Error(response.data.message);
      }
...
export const RequestCampus=(data)=>{
  ...
  return async(dispatch)=>{
    ...
    try{
      const response=await apiConnector(AdminApi.submitCampusRequestApi,"POST",data);
      if(!response.data.success){
        throw new Error(response.data.message);
      }
