...
import { getUserProfileDetails } from '../services/operations/Auth';
...
export default function ProfilePage() {
  ...
  const [jwtUserData, setJwtUserData] = useState(null);
  ...
  const fetchUserProfile = async () => {
    dispatch(getUserProfileDetails(token))
  };
  ...
  // Load profile from Redux store on component mount
  useEffect(() => {
    if (Profile) {
      setUserData(Profile);
      setProfileData({
      ...
      setProfileImage(Profile.profileImage || '/placeholder-user.jpg');
    }
  }, [Profile]);

  if (!user || !token) {
    return (
      <div className='min-h-screen w-full bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 flex items-center justify-center'>
