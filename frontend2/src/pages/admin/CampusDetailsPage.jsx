    ...
    async function fetchCampusDetails() {
      ...
      try {
        const [campusRes, canteensRes, usersRes] = await Promise.all([
          apiConnector(AdminApi.campusesSummaryApi, "GET", null, {
            Authorization: `Bearer ${token}`
          }),
          apiConnector(`${AdminApi.campusCanteensApi}/${campusId}/canteens`, "GET", null, {
            Authorization: `Bearer ${token}`
          }),
          apiConnector(`${AdminApi.campusUsersApi}/${campusId}/users`, "GET", null, {
            Authorization: `Bearer ${token}`
          }),
        ]);
        ...
        setError(err?.response?.data?.message || err?.message || "Failed to load campus details");
        toast.error(err?.response?.data?.message || err?.message || "Failed to load campus details");
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="max-w-7xl mx-auto p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <Card 
          ...
          onClick={() => navigate(`/admin/campuses/${campusId}/users`)}
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {canteens.map((canteen) => (
              <Card
                key={canteen._id}
                className="bg-white/10 border-white/20 hover:bg-white/20 transition-colors cursor-pointer"
                onClick={() => navigate(`/admin/canteens/${canteen._id}`)}
              >
