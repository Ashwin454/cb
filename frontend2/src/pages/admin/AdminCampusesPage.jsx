...
export default function AdminCampusesPage() {
        ...
        const campusesWithStudentCount = await Promise.all(
          campusSummaries.map(async (campus) => {
            try {
              ...
              const studentUsers = (usersRes.data.users || []).filter(
                (u) => u.role === "student"
              );
              return { ...campus, userCount: studentUsers.length };
            } catch (error) {
              console.error(
                `Failed to fetch users for campus ${campus.name}:`,
                error
              );
              return { ...campus };
            }
    ...
    async function fetchCampusRequests() {
      ...
      try {
        ...
        setRequestsError(
          err?.response?.data?.message ||
            err?.message ||
            "Failed to load campus requests"
        );
      } finally {
  ...
  async function handleAddCampus() {
    ...
    try {
      ...
      toast.error(
        err?.response?.data?.message || err?.message || "Failed to add campus"
      );
    } finally {
      setAddLoading(false);
    }
  ...
  async function handleApproveRequest(req) {
    ...
    try {
      ...
      const createRes = await apiConnector(
        CampusApi.createCampus,
        "POST",
        {
      ...
      const campusesRes = await apiConnector(
        AdminApi.campusesSummaryApi,
        "GET",
        null,
        {
  ...
  async function handleRejectRequest(req) {
    try {
      ...
      toast.error(
        err?.response?.data?.message ||
          err?.message ||
          "Failed to reject campus request"
      );
    }
  }
  ...
  return (
    <div className="max-w-7xl mx-auto p-8">
                ...
                filteredCampuses.map((campus) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2">
                      <Button
                        size="sm"
                        className="bg-black hover:bg-neutral-900 text-white font-semibold"
      ...
      <div className="mt-12 bg-white/10 rounded-xl p-6">
        ...
        ) : campusRequests.filter((r) => r.isReviewed === false).length ===
          0 ? (
          <div className="text-slate-400 py-8 text-center">
            No campus requests found.
          </div>
        ) : (
      ...
      <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
        <DialogContent className="max-w-md bg-slate-800 border-slate-600">
          <DialogHeader>
            <DialogTitle className="text-white">Add New Campus</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <Input
              ...
              className="bg-slate-700 border-slate-500 text-white placeholder-gray-300 focus:border-slate-400"
            />
            <Input
